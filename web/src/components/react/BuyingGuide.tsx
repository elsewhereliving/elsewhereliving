...
const LEGAL_GUIDES: Guide[] = [
  {
    ...
    secure: {
      ...
      layers: [
        ...
        { n: "04", t: "Usufruct — the right to use and profit", b: "Registered for your lifetime, giving you the legal right to use and enjoy the property — including receiving the income it generates, where permitted. Like a lease, it is a right to use the land, so it is chosen as an alternative rather than stacked on top — but where a lease has a hard 30-year edge, ==a usufruct runs as long as you do==." },
      ],
    ...
    process: [
      { t: "Reserve", b: "A reservation agreement and a deposit — up to around 15%, depending on the developer or owner — take the property off the market while diligence runs. Under the reservation agreement, ==the deposit is refundable if due diligence turns up material title issues.==" },
      { t: "Due diligence", b: "An independent lawyer verifies the title deed — ==Chanote (Nor Sor 4 Jor) is the gold standard==; lesser titles need real caution — plus encumbrances, access, building permits, and for off-plan, the developer's licences, EIA approval and track record. Due diligence normally takes one to four weeks." },
      { t: "Sale & purchase agreement", b: "Once due diligence is complete, the contracts are signed and ==a deposit of 30–35% is paid at the same time as signing==. Off-plan purchases then follow a staged payment schedule tied to construction milestones." },
    ...
    know: [
      { t: "Chanote or Nor Sor 3 — nothing less", b: "==Chanote (Nor Sor 4 Jor) and Nor Sor 3 Gor are both acceptable title deeds==, with Chanote — the only GPS-surveyed, full title — the gold standard. Nor Sor 3 Gor is a confirmed right of possession rather than full ownership, and its boundaries are less precisely surveyed, so it warrants closer checking — and lenders prefer Chanote. Anything below those two we simply don't list." },
      ...
      { t: "Check the quota before you fall in love", b: "In popular Samui and Phuket condos the 49% foreign quota can be full — Thai-quota units can only be bought leasehold or via company. We confirm quota in writing before you proceed." },
